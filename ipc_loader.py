...
def load_schematic(kicad_sch: str):
    ...
    try:
        ...
        print(
            "kicad-python not installed.\n"
            "Run: pip install kicad-python --break-system-packages"
        )
        sys.exit(1)

    try:
        client = kipy.KiCad()
        client.open_schematic(str(kicad_sch))
        print(f"Loaded into KiCad: {kicad_sch}")
    except Exception as e:
        print(f"kipy call failed ({e}), trying raw IPC...")
        _raw_open(str(kicad_sch))
